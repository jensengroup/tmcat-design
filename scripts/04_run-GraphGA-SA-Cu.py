...
from suzuki import SuzukiCatalystCu
...
class GraphGA(GA):
    ...
    def make_initial_population(self) -> List[SuzukiCatalystCu]:
        ...
        for line in lines:
            smi = line.rstrip()
            cat = SuzukiCatalystCu.from_smiles(smi)
            cat.metal = Metal("Cu+")
    ...
    def mutate(ind):
        ...
        counter = 0
        while not new_mol:
            new_mol = graph_mutate(ind.ligands[idx].mol, allow_change_charge=False)
            counter += 1
            if counter > 10:
                return None
        try:
...
if __name__ == "__main__":
    ...
    mol_options = MoleculeOptions(
        individual_type=SuzukiCatalystCu,
    )

    scoring_args = {
        "adjecency_check": True,
        "use_hueckel": True,
    ...
    ga = GraphGA(
        ...
        n_generations=50,
        mutation_rate=0.75,
        db_location="../data/ga_runs/07_graphGACu+SA.sqlite",
        scoring_args=scoring_args,
